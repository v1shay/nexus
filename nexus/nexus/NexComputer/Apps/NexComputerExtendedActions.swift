...
actor NexGitHubActionCatalog {
    ...
    func register(on registry: NexComputerRegistry) async throws {
        ...
        for (id, args) in [("git.status", ["status", "--porcelain=v2", "--branch"]), ("git.diff", ["diff", "--no-ext-diff", "--"])] {
            try await registry.register(manifest: Self.manifest(id, id == "git.status" ? "Read structured Git repository branch and working-tree state." : "Read the current repository diff without external diff drivers.", [id == "git.status" ? "Show git status" : "Show the current diff"], ["repository": .init(.string, required: true)])) { input, _ in guard let path = input["repository"]?.string else { throw NexToolError.missingField("repository") }; let result = try await cli.git(args, repository: URL(fileURLWithPath: path)); return Self.result(result.stdout.isEmpty ? "No changes." : result.stdout, output: result.stdout) }
        }
