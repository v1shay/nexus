...
enum NexPrimaryToolPlanner {
    ...
    static func nativePlanningMessages(
        ...
        if names.contains("browser.play_youtube") {
            rules.append("- browser.play_youtube: use directly when Sir asks Nexus to play something, play YouTube, or open YouTube and start a video. Supply query only when he names what to play. This special browser tool—not youtube_search/youtube_play—opens Nexus's own browser visibly, starts the first result, and leaves it playing.")
        }
    ...
    static func planningMessages(
        ...
        For media control, never answer as though playback happened without using the matching YouTube tool. If `browser.play_youtube` is available and Sir asks Nexus to play something, open YouTube, or play a named YouTube request, call it directly. It is intentionally different from existing Chrome playback: it opens the signed-in Nexus browser in front, clicks the first visible result, waits for normal YouTube playback, and leaves that browser open. Do not try to write Playwright steps for this special case. If Sir asks to play, show, or continue the video already open in Google Chrome, call youtube_play_current with no arguments. If browser.play_youtube is unavailable and Sir asks Nex to find and play a YouTube video, first call youtube_search with a standalone, descriptive query; inspect its returned candidates on the next planning pass, then call youtube_play using exactly one returned video_id. If the user asks to make an already playing Nex YouTube video bigger or full screen, call youtube_fullscreen with no arguments. These tools own playback and the overlay; do not use web_search as a substitute for YouTube playback.
