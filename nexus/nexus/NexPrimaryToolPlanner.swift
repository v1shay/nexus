...
enum NexPrimaryToolPlanner {
    ...
    static func nativePlanningMessages(
        ...
        if names.contains("github.search") {
            rules.append("- github.search: use it for public GitHub repository, issue, or pull-request searches through the installed authenticated CLI. Give `query` a complete search phrase and use `type: repositories` unless the user specifically asks for issues or pull requests.")
        }
        ...
        if names.contains("browser.play_youtube") {
            rules.append("- browser.play_youtube: use directly for vague playback commands such as play something, open YouTube, or add more. Supply query only when Sir names what to play. This special browser tool opens Nexus's own browser visibly, makes the browser and video full screen, starts the first result, and leaves it playing. A researched selection still uses youtube_search followed by youtube_play; youtube_play hands its exact returned video_id to this same browser presentation path.")
        }
        if names.contains("browser.open_schoology") {
            rules.append("- browser.open_schoology: use only when Sir asks to open, show, or go to Schoology. It selects the saved FUHSD Google session when needed, brings the Nexus browser forward, and makes it full screen. Do not use it for an assignment summary.")
        }
        if names.contains("browser.check_schoology") {
            rules.append("- browser.check_schoology: use directly for checking Schoology, school homework, or upcoming assignments. It reads only live evidence from the signed-in Nexus browser profile. Supply school_email only if Sir has given the exact school address; never invent one.")
        }
        ...
        if names.contains("youtube_play") {
            rules.append(
                "- youtube_play requires the exact video_id returned by youtube_search in prior conversation context. It presents that exact result in the same frontmost full-screen Nexus browser used by browser.play_youtube. If no returned video_id is present, do not substitute youtube_play_current and do not invent an ID."
            )
        }
    ...
    static func planningMessages(
        ...
        For media control, never answer as though playback happened without using the matching YouTube tool. If Sir gives a vague playback command such as play something, open YouTube, or add more, call `browser.play_youtube`; it opens the signed-in Nexus browser in front, makes the browser full screen, clicks the first visible result, waits for normal YouTube playback, makes the video full screen, and leaves that browser open. If Sir asks Nexus to actively find or choose a video about a subject, first call youtube_search with a standalone descriptive query, inspect its returned candidates on the next planning pass, then call youtube_play using exactly one returned video_id; youtube_play opens that exact selection through the same frontmost full-screen Nexus browser path. Do not write Playwright steps for either special case. If Sir asks to play, show, or continue the video already open in Google Chrome, call youtube_play_current with no arguments. If the user asks to make an already playing Nex media-overlay video bigger or full screen, call youtube_fullscreen with no arguments. Do not use web_search as a substitute for YouTube playback.
