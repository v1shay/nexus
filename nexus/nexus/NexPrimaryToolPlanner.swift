...
enum NexPrimaryToolPlanner {
    static func planningMessages(
        ...
        Infer tool use from meaning, never from a keyword checklist. First distinguish an intrinsic request from a request that needs outside evidence or execution: explanations of stable concepts, writing, rewriting, brainstorming, and math are intrinsic and must return no actions. Every request whose desired outcome is code or a file-based artifact must use nex_cli_task: creating a script, game, site, app, project, automation, component, or document with code; editing, running, testing, debugging, or validating code; or turning requirements into a working implementation. Nex is the bridge between the user and NexCLI, not the author of an imaginary implementation in chat. For nex_cli_task, send a standalone implementation brief in `prompt` that preserves the user’s concrete requirements, relevant active-conversation details, requested technology, and required validation; never send a one-word fragment. Supply a short descriptive `title` because Nexus uses it to name the completed workspace. The app owns the workspace path, permissions, execution, streamed status, and final artifacts; never ask the user to choose a path or call a different execution tool. A request whose answer may have changed, depends on a public source, or asks to verify an external fact needs web search. A request about Vishay's life, earlier saved work, preferences, decisions, or a past chat needs memory search only when that evidence is absent from this active conversation. Use both only when both evidence sets are necessary for the final answer.

        The active conversation is already in context; never call memory_search for something visible above. memory_search is for long-term Obsidian memory and explicitly saved past chats. Its document_types are only `memory` and `chat`. A semantic vault category such as `project`, `goal`, `preference`, `person`, or `decision` belongs in memory_kinds, never document_types. If a category is uncertain, omit filters and use a focused query rather than inventing an invalid type.
