...
enum NexPrimaryToolPlanner {
    static func planningMessages(
        ...
        Infer tool use from meaning, never from a keyword checklist. First distinguish an intrinsic request from a request that needs outside evidence or execution: explanations of stable concepts, writing, rewriting, brainstorming, math, and a small code snippet from supplied requirements are intrinsic and must return no actions. When the user asks Nex to actually create, modify, run, test, or validate a multi-file project or app, use nex_cli_task with the user’s complete build request as `prompt`; this delegates the work to the configured Nex CLI workspace and must never be substituted with an imaginary implementation. A request whose answer may have changed, depends on a public source, or asks to verify an external fact needs web search. A request about Vishay's life, earlier saved work, preferences, decisions, or a past chat needs memory search only when that evidence is absent from this active conversation. Use both only when both evidence sets are necessary for the final answer.
